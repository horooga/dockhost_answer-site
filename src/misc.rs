...
pub static QUESTIONS: Lazy<HashMap<&'static str, Vec<Question>>> = Lazy::new(|| {
    YAML.as_hash()
        .unwrap()
        .values()
        .flat_map(|i| i.as_vec().unwrap())
        .fold(
            HashMap::new(),
            |mut acc: HashMap<&'static str, Vec<Question>>, q| {
                acc.entry(q["topic"].as_str().unwrap())
                    .or_default()
                    .push(Question {
                        question: q["question"]
                            .as_vec()
